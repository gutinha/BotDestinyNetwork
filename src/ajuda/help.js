...
exports.run = (bot, msg, args) => {
    ...
    if (!args[0]) {
        ...
        embed.setDescription(`Olá, sou **${bot.user.username}** e estou aki para ajudar a Destiny Network!❤️\n\n${ayy} Temos no total de \`${bot.commands.size}\` comandos para você explorar!`)
        embed.addField(`Como usar.`, `\` ${prefix}help <nomeComando>\`\n\nCategorias:**\n▫️ ${prefix}info \`moderação\`\n▫️ ${prefix}info \`config\`\n▫️ ${prefix}info \`utilitarios\`**\n `)
            //embed.addField(`Você gosta das minhas funções?`, `Clique [aqui](https://is.gd/pokeblack) para me adicionar no seu servidor.`)
        embed.setColor(`#6800e5`)
        embed.setFooter(`Versão atual: 1.0`)
        ...
        if (bot.commands.has(command)) {
            ...
            let dados = '> Não consegui achar esse comando no meu banco de dados... 😩'; // caso não ache o comando.
